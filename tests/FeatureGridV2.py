...
class FeatureGridV2:
    """
    A FeatureGrid is defined four attributes that describe a N-dimensional
    array with C=N-1 coordinate dimensions and 1 feature dimension so that
    the data shape is (1, ... ,C, F), with the feature axis last by convention.

    Each coordinate dimension is named by a unique string, and is paired with
    a 1-dimensional monotonic array with the same size as the dimension. The
    coordinate array may be used for point or range searching by coordinate
    value.

    :@param data: New file path or numpy array
    :@param flabels: Tuple of unique strings giving names to each element in
        the final dimension of the data array.
    :@param clabels Tuple of unique strings giving names to each dimension
        preceding the final dimension.
    :@param coords:
    :@param masks:
    :@param meta:
